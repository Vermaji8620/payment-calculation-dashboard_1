...
const memoryCache = {
  expenses: { data: null, timestamp: 0 }
};
...
export async function ensureExpenseTable() {
  ...
  await sql`CREATE INDEX IF NOT EXISTS idx_expense_entries_created_at ON expense_entries(created_at)`.catch(() => {});
}
...
export async function listExpenses() {
  ...
  memoryCache.expenses = { data, timestamp: Date.now() };
  return data;
}
